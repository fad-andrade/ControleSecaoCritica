...
public class Car implements Runnable{
    ...
    public void bridgeCrossing(int xposOld){
        while(cartype == REDCAR && !(xpos >= bridgeXRight &&  xposOld < bridgeXRight)) {
            xpos += 4;
        ...
        if((cartype == REDCAR) && (xpos >= bridgeXRight &&  xposOld < bridgeXRight)){
            controller.unlockMutex();
        }

        while(cartype == BLUECAR && !(xpos <= bridgeXLeft && xposOld > bridgeXLeft)) {
            xpos -= 4;
        ...
        if((cartype == BLUECAR) && (xpos <= bridgeXLeft && xposOld > bridgeXLeft)){
            controller.unlockMutex();
        }
    }
